...
// VAL-01/02/03: Visit Type, Purpose of Visit, and Visit Zone are all required on the Visit Info
// step. Confirmed directly (not guessed): leaving any one empty disables "Next" (native disabled
// attribute) and shows an inline error under that specific field - the same mechanism for all
// three, so one parameterized test covers all of them, per the Test Strategy doc's own
// recommendation to use a DataProvider when only the input varies.
@Epic("Visit Management")
@Feature("Validation")
public class CreateVisitMissingRequiredFieldTest extends BaseTest {
    ...
    public Object[][] missingRequiredField(){
        return new Object[][]{
                {"visitType", "Visit Type is required"},
                {"purpose", "Purpose of visit is required"},
                {"visitZone", "Visit Zone is required"},
        };
    }

    @Story("Visit Info Required Field Validation")
    @Severity(SeverityLevel.NORMAL)
    @Test(dataProvider = "missingRequiredField",
            description = "Next Is Blocked When A Required Visit Info Field Is Missing",
            groups = {"negative", "regression", "visit", "regular"})

    public void createVisitFailsWhenRequiredFieldMissing(String fieldToSkip, String expectedError) {
        ...
        if (!fieldToSkip.equals("visitType")) {
            visitsPage.selectVisitType("Maintenance");
        } else {
            visitsPage.touchVisitTypeField();
        }
        if (!fieldToSkip.equals("purpose")) {
            visitsPage.enterPurposeOfVisit(new UserUtils().generateRandomPurpose());
        } else {
            visitsPage.touchPurposeField();
        }
        if (!fieldToSkip.equals("visitZone")) {
            visitsPage.selectVisitZone(visitZone);
        } else {
            visitsPage.touchVisitZoneField();
        }

        Assert.assertTrue(visitsPage.isNextButtonDisabled(),
                "Next button should stay disabled while '" + fieldToSkip + "' is missing");
        Assert.assertTrue(visitsPage.isValidationErrorDisplayed(expectedError),
                "Expected validation error not shown: " + expectedError);
    }
}
